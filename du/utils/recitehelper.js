...
class ReciteHelper {
  ...
  constructor(pageData, pageSetDataFn) {
    ...
    // 读取储存着的笔记
    let recites = JSON.parse(util.getStorageSync('recites') || '[]');
  ...
  //停止播放声音，参数为对象
  stopVoice(key) {
    const recites = this.recites.filter(v => v.key !== key);
...
showVoiceActions (voicekey) {
  ...
  */
  ...
  recognizeVoice(key, path) {
    util.uploadRecognizeVoice({
      ...
      success: res => {
        ...
        if (!result.hasError) {
          const recites = this.recites.map(v => {
            if (v.key === key) {
              v.word = result.text;
              v.isRec = true;
            }
  ...
  uploadRecite(key) {
    const recite = this.recites.filter(v => v.key !== key);
    if (recite.length == 0)
      return;
